...
```{r}
# New methods for task 2
df3 <- ltv 
df3$date <- ymd(df3$date)

df3 <- df3 %>%
  group_by(id) %>% 
  dplyr::mutate(duration = ifelse(date == min(date), 1, ifelse(status==2, ceiling((((date) - min(date))/30)), ceiling((as.Date("2014-12-31") - min(date))/30))), recency = ifelse(status==2, round((as.Date("2014-12-31")-max(date))/30,digits = 1),0))

# test for task 2
df4 <- df3 %>% 
  group_by(id,gender) %>% 
  dplyr::summarize(page.m = sum(pages)/max(duration), onsite.m = sum(pages)/max(duration), enter.m = sum(entered)/max(duration), completed.m = sum(completed)/max(duration), holiday.m = sum(holiday)/max(duration), duration = max(duration), recency = max(recency), frequency = n())

#First split the dataset into a training set and a test set
data(df4)
## 70% of the sample size
smp_size <- floor(0.7 * nrow(df4))
## set the seed to make your partition reproductible
set.seed(123)
train_ind <- sample(seq_len(nrow(df4)), size = smp_size)
ltv.train <- df4[train_ind, ]
ltv.test <- df4[-train_ind, ]

# Fit 
set.seed(1)
testtest = randomForest(duration~., data=ltv.train)
y.hat=as.numeric(y.hat)
test.y = ltv.test$duration
mean((y.hat-test.y)^2)
