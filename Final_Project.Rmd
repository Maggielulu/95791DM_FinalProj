...
df.2 = ddply(ltv,c("id","gender"),summarize, cancelled = max(status), days.count = as.numeric(max(as.Date(date))-min(as.Date(date))),page.visited=sum(pages),time.spent=sum(onsite),entered.time = sum(entered), cmplt.time = sum(completed),holiday = max(holiday))
df.2$cancelled = mapvalues(df.2$cancelled, from=c('0','1','2'),to=c(0,0,1))
df.2$page.pm = df.2$page.visited/df.2$days.count*30
df.2$time.pm = df.2$time.spent/df.2$days.count*30
df.2$entered.pm = df.2$entered.time/df.2$days.count*30
df.2$cmplt.pm = df.2$cmplt.time/df.2$days.count*30
ltv.2 = df.2[,c(2,3,4,9,10,11,12,13)]
```
